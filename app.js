...
function renderManageItems() {
    ...
    // Obtener datos actuales (asegurando que existan)
    ...
    // --- SECCIÓN DE ÁREAS ---
    html += '<h3 class="font-bold text-lg mb-2 text-white">Áreas</h3>';
    ...
    if (areas.length > 0) {
        areas.forEach((area, index) => {
            ...
            <div class="flex items-center justify-between bg-gray-800 p-2 rounded border border-gray-700">
                <span class="text-sm text-gray-200">${escapeHtml(area.name)}</span>
                <div class="space-x-2">
                    <button onclick="editArea(${index})" class="text-xs bg-blue-600 hover:bg-blue-500 text-white px-2 py-1 rounded">Editar</button>
                    <button onclick="deleteArea(${index})" class="text-xs bg-red-600 hover:bg-red-500 text-white px-2 py-1 rounded">Eliminar</button>
                </div>
            </div>`;
        });
    } else {
        html += '<p class="text-xs text-gray-400 italic">No hay áreas definidas.</p>';
    }
    html += '</div>';
    
    // Botón agregar área
    html += `<button onclick="addArea()" class="w-full mb-6 bg-green-700 hover:bg-green-600 text-white text-sm py-2 rounded font-bold">+ Nueva Área</button>`;

    // --- SECCIÓN DE CONTEXTOS ---
    html += '<h3 class="font-bold text-lg mb-2 text-white">Contextos</h3>';
    html += '<div class="mb-4 space-y-2">';
    
    if (contexts.length > 0) {
        contexts.forEach((ctx, index) => {
            // Usar color definido o uno por defecto
            const colorClass = ctx.color || 'bg-gray-500';
            html += `
            <div class="flex items-center justify-between bg-gray-800 p-2 rounded border border-gray-700">
                <div class="flex items-center gap-2">
                    <span class="w-3 h-3 rounded-full ${colorClass}"></span>
                    <span class="text-sm text-gray-200">${escapeHtml(ctx.name)}</span>
                </div>
                <div class="space-x-2">
                    <button onclick="editContext(${index})" class="text-xs bg-blue-600 hover:bg-blue-500 text-white px-2 py-1 rounded">Editar</button>
                    <button onclick="deleteContext(${index})" class="text-xs bg-red-600 hover:bg-red-500 text-white px-2 py-1 rounded">Eliminar</button>
                </div>
            </div>`;
        });
    } else {
        html += '<p class="text-xs text-gray-400 italic">No hay contextos definidos.</p>';
    }
    html += '</div>';

    // Botón agregar contexto
    html += `<button onclick="addContext()" class="w-full bg-green-700 hover:bg-green-600 text-white text-sm py-2 rounded font-bold">+ Nuevo Contexto</button>`;

    // Insertar HTML generado
    container.innerHTML = html;
}

// --- FUNCIONES AUXILIARES NECESARIAS (Agrega estas si no existen) ---
...
function addArea() {
    const name = prompt("Nombre del nueva Área:");
    if (!name) return;
    if (!window.areas) window.areas = [];
    window.areas.push({ name: name });
    saveConfigAndRender();
}

function editArea(index) {
    const area = window.areas[index];
    const newName = prompt("Editar nombre del Área:", area.name);
    if (!newName) return;
    window.areas[index].name = newName;
    saveConfigAndRender();
}

function deleteArea(index) {
    if(!confirm("¿Eliminar esta área?")) return;
    window.areas.splice(index, 1);
    saveConfigAndRender();
}
...
function addContext() {
    ...
    // Asignar color aleatorio simple si no se pide uno
    const colors = ['bg-red-500', 'bg-blue-500', 'bg-green-500', 'bg-yellow-500', 'bg-purple-500', 'bg-pink-500'];
    const randomColor = colors[Math.floor(Math.random() * colors.length)];
    
    window.contexts.push({ name: name, color: randomColor });
    saveConfigAndRender();
}

function editContext(index) {
    const ctx = window.contexts[index];
    const newName = prompt("Editar nombre del Contexto:", ctx.name);
    if (!newName) return;
    
    // Simple prompt para color (podría mejorarse con un picker)
    const newColor = prompt("Clase de color (ej: bg-red-500):", ctx.color || 'bg-gray-500');
    
    window.contexts[index] = { name: newName, color: newColor || 'bg-gray-500' };
    saveConfigAndRender();
}

function deleteContext(index) {
    if(!confirm("¿Eliminar este contexto?")) return;
    window.contexts.splice(index, 1);
    saveConfigAndRender();
}

// Función genérica para guardar configuración y refrescar
function saveConfigAndRender() {
    // Asumiendo que existe una función saveConfig o similar en tu app original.
    // Si tu app usa 'saveSettings()' o 'updateSheet()', llámala aquí.
    // Por seguridad, llamamos a la función de guardado principal si existe, 
    // o forzamos el renderizado localmente.
    
    if (typeof saveSettings === 'function') {
        saveSettings(); 
    } else if (typeof syncDataToSheet === 'function') {
        syncDataToSheet();
    }
    ...
    // Opcional: Recargar datos desde sheet si es necesario
